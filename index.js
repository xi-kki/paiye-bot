...
bot.on('message', async (msg) => {
  ...
  await bot.sendMessage(chatId,
    `Hey! I'm *Paiye* — your AI career agent 🤖\n\n` +
...
async function cmdStart(msg) {
  ...
  await bot.sendMessage(chatId,
    ...
    `*What I can do:*\n` +
    `• 🔍 /search \\<query\\> — Search in plain English (e.g., \`/search senior AI engineer\`)\n` +
...
async function cmdHelp(msg) {
  await bot.sendMessage(msg.chat.id,
    ...
    '• 💬 /ai — Chat about anything (career, tech, casual)\n\n' +
    '*📄 Resume & Career*\n' +
...
async function main() {
  ...
  // Register clean command list with Telegram
  try {
    await bot.setCommands([
      { command: 'start', description: 'Main menu' },
      { command: 'search', description: 'Search jobs in plain English' },
      { command: 'nigeria', description: 'Remote jobs open to Nigeria' },
      { command: 'resume', description: 'Upload your CV for ATS analysis' },
      { command: 'analyze', description: 'Get AI feedback on your resume' },
      { command: 'interview', description: 'Prep for an interview (send job URL)' },
      { command: 'tailor', description: 'Tailor resume to a job (send job URL)' },
      { command: 'profile', description: 'Set your job preferences' },
      { command: 'ai', description: 'Chat with AI about careers & tech' },
      { command: 'subscribe', description: 'Get 15 jobs daily at 7AM WAT' },
      { command: 'unsubscribe', description: 'Stop daily job delivery' },
      { command: 'feedback', description: 'Share feedback or suggestions' },
      { command: 'refresh', description: 'Force re-fetch all job sources' },
      { command: 'help', description: 'Show complete command guide' }
    ]);
    await bot.setDescription('Paiye — AI Career Agent. Search remote jobs, ATS resume analysis, interview prep, AI chat. Find your next role in tech, writing, data & more.');
    await bot.setShortDescription('🤖 AI Career Agent — remote job search, ATS analysis, interview prep');
    console.log('✅ Telegram command list updated');
  } catch (err) {
    console.log('⚠️ Could not set commands:', err.message);
  }
